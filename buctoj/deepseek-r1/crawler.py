...
def perform_login(login_page_url, login_process_url, http_session, user_id, user_password):
    try:
        print(f"登录页URL: {login_page_url}")  # 调试输出登录URL
        # 访问登录页，获取 CSRF token
        login_page_response = http_session.get(login_page_url)
        print("登录页内容前200字符:", login_page_response.text[:200])  # 调试输出登录页内容
...
def fetch_problem_links(oj_base_url, contest_id, http_session, contest_page_url):
    # 获取页面内容
    print(f"正在访问的URL: {contest_page_url}")  # 调试输出URL
    ...
    # 调试输出部分页面内容
    print("页面内容前500字符:", response.text[:500])
    ...
    for link_element in soup.select('table.ui.table a[href^="problem.php?cid="]'):
