...
def main(n):
    """
    n: problem scale, here interpreted as the array length.
    We generate:
        m in [1, n] (and m divides n when possible to keep t integer),
        a[i] random in [0, 1_000_000].
    Then we run the original logic and print the result.
    """
    if n <= 0:
        return

    # choose m (try to make n % m == 0; if not possible, just pick between 1 and n)
    divisors = [d for d in range(1, n + 1) if n % d == 0]
    if divisors:
        m = random.choice(divisors)
    else:
        m = random.randint(1, n)

    # generate test data for a: values in a range that makes a[i] % m varied
