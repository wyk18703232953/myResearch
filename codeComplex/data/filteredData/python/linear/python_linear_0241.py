def main(n):
    # n controls string length; turns is fixed to 3 for deterministic experiments
    turns = 3
    alphabet = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz'
    k = len(alphabet)

    # deterministically generate three strings of length n over the alphabet
    s0 = ''.join(alphabet[i % k] for i in range(n))
    s1 = ''.join(alphabet[(i * 2 + 1) % k] for i in range(n))
    s2 = ''.join(alphabet[(i * 3 + 2) % k] for i in range(n))
