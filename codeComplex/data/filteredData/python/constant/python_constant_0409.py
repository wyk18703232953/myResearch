...
def main(n):
    # n controls the coordinate magnitude; use a deterministic pattern
    k = max(1, n)

    # Generate 8 points deterministically from n
    # First quadrilateral points A,B,C,D
    x0, y0 = 0, 0
    x1, y1 = k, 0
    x2, y2 = k, k
    x3, y3 = 0, k

    # Second quadrilateral points a,b,c,d (shifted and scaled versions)
    x4, y4 = k // 2, k // 2
    x5, y5 = k + k // 2, k // 2
    x6, y6 = k + k // 2, k + k // 2
    x7, y7 = k // 2, k + k // 2
