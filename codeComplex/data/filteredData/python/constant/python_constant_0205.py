...
# ---- Utility library (unchanged logical content, just cleaned) ----

def divisors_simple(M):
    d = []
    i = 1
    while M >= i * i:
        if M % i == 0:
...
def xorconv(n, X, Y, mod, inv):
    ...
    res1 = xorconv(n - 1, x, y, mod, inv)
    res2 = xorconv(n - 1, z, w, mod, inv)
    ...
    def __init__(self, A, merge_func, ide_ele):
        ...
        for j in range(1, n):
            for i in range(0, N - (1 << j) + 1):
                f = self.table[i][j - 1]
                s = self.table[i + (1 << (j - 1))][j - 1]
                self.table[i][j] = self.merge_func(f, s)

    def query(self, s, t):
        b = t - s + 1
        m = b.bit_length() - 1
        return self.merge_func(self.table[s][m], self.table[t - (1 << m) + 1][m])
...
class Matrix:
    ...
    def __add__(self, other):
        ...
        for i in range(self.row):
            for j in range(self.column):
                res[i][j] = (self._matrix[i][j] + other._matrix[i][j]) % Matrix.mod
        return Matrix(res)
...
# We generate k as a list of 3 integers based on n.
# Interpretation: use n as max value of each element; keep size fixed = 3.

def main(n: int):
    # 3 numbers in [1, max(3, n)] so that small n is still meaningful
    hi = max(3, n)
    k = [random.randint(1, hi) for _ in range(3)]
    k.sort()
    k = tuple(k)

    ans_patterns = [(2, 4, 4), (3, 3, 3)]
    if k.count(1) >= 1:
        print("YES")
    elif k.count(2) >= 2:
        print("YES")
