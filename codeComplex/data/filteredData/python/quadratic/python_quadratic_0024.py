...
def generate_test_data(M, N):
    # 生成 N 行、每行长度为 M 的随机字符串，只含小写字母
    alphabet = string.ascii_lowercase
    S = [''.join(random.choice(alphabet) for _ in range(M)) for _ in range(N)]
    return M, N, S
...
def main(n):
    # n 为规模参数，这里使用 n 同时作为 M 和 N
    M = n
    N = n
    M, N, S = generate_test_data(M, N)
    return solve(M, N, S)

if __name__ == "__main__":
    # 示例：直接调用 main(5) 进行一次运行并打印结果
    print(main(5))