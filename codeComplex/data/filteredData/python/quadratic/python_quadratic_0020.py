...
def main(n):
    # Interpret n as both number of rows N and number of columns M
    # to scale a square-like input: M = N = n (with n >= 1)
    if n <= 0:
        n = 1
    M = n
    N = n

    # Deterministically generate S: N strings, each of length M
    # Use simple arithmetic patterns to fill characters.
    # Map values to lowercase letters in a fixed way.
    S = []
    for j in range(N):
        row_chars = []
        for i in range(M):
            val = (i * (j + 1) + j * j + i // 2) % 26
            row_chars.append(chr(ord('a') + val))
