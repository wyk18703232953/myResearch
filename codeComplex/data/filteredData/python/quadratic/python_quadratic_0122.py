...
def main(n):
    """
    n: 规模。生成一组测试数据并输出原逻辑的结果。
    原逻辑:
      读入 n, m; 再读入长度为 m 的数组 a (1-based, 值在 [1, n])。
      统计每个 1..n 出现次数，输出最小出现次数。
    这里:
      - 固定 m = n，生成一个长度为 n 的随机数组 a，元素范围 [1, n]。
    """
    if n <= 0:
        print(0)
        return
