...
def deterministic_matrix(n):
    # Generate an n x n matrix of characters deterministically.
    # Pattern: mat[i][j] = chr(ord('a') + (i + j) % 26)
    return [[chr(ord('a') + (i + j) % 26) for j in range(n)] for i in range(n)]


def main(n):
    a1_mat = deterministic_matrix(n)
    a2_mat = deterministic_matrix(n)
...
if __name__ == "__main__":
    ...
    # You can change 10 to any other n to scale the input size.
    main(10)