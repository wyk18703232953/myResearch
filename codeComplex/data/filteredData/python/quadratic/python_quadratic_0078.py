...
def decode(bit_string: str) -> str:
    """Decode a single 8-bit (LSB-first) string into the corresponding output."""
    idx = int("0b" + ''.join(reversed(bit_string)), 2)
    s = dat[idx]
    for i in range(8):
        s = s.replace(d1[i], d2[i])
    return s
