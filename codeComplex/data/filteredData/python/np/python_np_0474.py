...
# Precompute primes once, deterministically up to a fixed bound
PRIMES = [i for i in range(2, 10**5) if t_prime(i)]
PSET = set(PRIMES)

def run_algorithm(n, k, l):
    # Local copies of primes and set so each run is independent
    primes = list(PRIMES)
    pset = set(PSET)
...
def main(n):
    # Input structure in original program:
    # first line: two integers n, k
    # second line: n integers l[0..n-1]
    #
    # Here we interpret the "scale" parameter n as:
    # - list length = n
    # - k = n (clamped to at least 1 and at most n to keep it meaningful)
    size = max(1, n)
    k = size
    # Deterministic generation of l:
    # l[i] = i + 2, ensures positive integers and some shared factors
    l = [i + 2 for i in range(size)]
    result = run_algorithm(size, k, l)
    print(result)
