...
def main(k):
    # 1. 生成测试数据：k 个序列，长度和数值可按需要调整
    # 这里生成长度在 [1, k] 之间、元素在 [-10^4, 10^4] 的随机整数
    n = []
    a = []
    for _ in range(k):
        ni = random.randint(1, max(1, k))
        n.append(ni)
        ai = [random.randint(-10_000, 10_000) for _ in range(ni)]
        a.append(ai)
...
def find_cycle(i_origin, j_origin, i, j, k, n, a, sums, tsum, num_map, mask, path):
    ...
    return find_cycle(
        i_origin, j_origin, i_next, j_next, k, n, a, sums, tsum, num_map, mask, path
    )
