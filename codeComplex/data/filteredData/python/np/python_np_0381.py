...
def generate_input(n):
    # n controls the number of groups k
    k = max(1, n)
    a = []
    base = 1
    for i in range(k):
        length = (i % 4) + 1  # lengths between 1 and 4
        group = []
        for j in range(length):
            group.append(base + i * 3 + j * 2)
        a.append(tuple(group))
        base += length
    return k, a
...
def reconstruct_output(dp, mask2i2cp):
...
def main(n):
    ...
    if totala % k != 0:
        ...
        result = reconstruct_output(dp, mask2i2cp)
