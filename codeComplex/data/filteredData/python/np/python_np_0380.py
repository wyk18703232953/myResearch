...
def generate_input(n):
    # n is the number of sequences (k)
    # For determinism, build sequences of deterministic lengths and values.
    # Sequence i has length i+1, values are arithmetic progression depending on n and i.
    k = max(1, n)
    a = []
    base = n
    for i in range(k):
        ...
        for j in range(length):
            # Deterministic value depending on (n, i, j)
            val = base + i * (i + 1) // 2 + j
            seq.append(val)
        a.append(tuple(seq))
    return k, a
