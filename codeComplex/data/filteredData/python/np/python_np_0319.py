        ...
        for j in range(1, k + 2):
            for l in range(4):
                ...
                if l == 0 or l == 3:
                    ...
                    new[j][l] = (((dp[j][1] % mod + dp[j][2] % mod) % mod) + new[j][l] % mod) % mod
                elif l == 1 or l == 2:
                    new[j][l] = (((dp[j - 1][0] % mod + dp[j - 1][3] % mod) % mod) + new[j][l] % mod) % mod
                    if j - 2 >= 0:
...
def main(n):
    if n < 1:
        ...
        n_use = n
    k = max(1, min(n_use, 2000))
    result = core_algorithm(n_use, k)
    print(result)
