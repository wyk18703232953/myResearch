...
def dfs(i: int, currentSum: int, arr, searchSum: int):
...
def run_core(atoms, outAtoms):
    ...
    resultFound = False
    def dfs2(i: int):
        nonlocal resultFound, stack, currentSet
        if i >= len(allCombos):
            resultFound = True
            return
        for s in allCombos[i]:
            if not s & currentSet:
                stack.append(s)
...
def main(n):
    if n < 1:
        n = 1
    atoms_indices = [(i % 100) + 1 for i in range(n)]
    out_indices = [((i * 2) % 100) + 1 for i in range((n + 1) // 2)]
    atoms = [numbersDict[str(i)] for i in atoms_indices]
    outAtoms = [numbersDict[str(i)] for i in out_indices]
    run_core(atoms, outAtoms)
