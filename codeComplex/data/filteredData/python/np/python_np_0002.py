...
def solve(xs, ys, n, oLst):
    def solveEven(seen):
        def solveEvenRec(notSeen, tsLst_local):
            if len(notSeen) == 0:
                return 0, []
            ns = tuple(sorted(notSeen))
            if ns in solveEvenRec.d:
                return solveEvenRec.d[ns]
            minLst = []
            minT = 10000000

            cands = getCandidates([pair for pair, _ in tsLst_local], len(notSeen), notSeen)
            for x in cands:
                i, j = x
