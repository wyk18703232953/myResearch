...
# -*- coding: utf-8 -*-
"""
更新LLM结果JSON文件，根据data目录下的实际Python文件过滤或更新记录
"""
import json
import os
import sys
...
def update_llm_results():
    ...
    if not os.path.exists(LLM_RESULTS_PATH):
        print(f"LLM结果文件不存在: {LLM_RESULTS_PATH}")
        return False
    ...
    print(f"JSON文件中包含 {len(detailed_records)} 条记录")
    
    # 过滤记录，只保留存在对应Python文件的记录
    filtered_records = []
    for record in detailed_records:
        complexity_id = record.get("complexity_id")
        if complexity_id in existing_files:
