...
def generate_ast(src):
    """生成代码的抽象语法树（AST）文本表示"""
    try:
        tree = ast.parse(src)
        return ast.dump(tree, indent=2)
    except Exception as e:
        return f"AST生成失败: {str(e)}"

def generate_cfg_representation(src):
    """生成代码的控制流图（CFG）文本表示
    
    注：这里使用简化的CFG表示，实际完整CFG需要更复杂的分析
    """
    try:
        # 解析AST
        tree = ast.parse(src)
        ...
        class CFGVisitor(ast.NodeVisitor):
            ...
            def visit_FunctionDef(self, node):
                self.nodes.append(f"函数定义: {node.name}()")
                super().visit_FunctionDef(node)
            
            def visit_If(self, node):
                self.nodes.append(f"条件分支: if语句")
                super().visit_If(node)
            
            def visit_For(self, node):
                self.loop_depth += 1
                self.nodes.append(f"循环: for循环 (深度: {self.loop_depth})")
                super().visit_For(node)
                self.loop_depth -= 1
            
            def visit_While(self, node):
                self.loop_depth += 1
                self.nodes.append(f"循环: while循环 (深度: {self.loop_depth})")
                super().visit_While(node)
                self.loop_depth -= 1
            
            def visit_With(self, node):
                self.nodes.append(f"上下文管理器: with语句")
                super().visit_With(node)
            
            def visit_Try(self, node):
                self.nodes.append(f"异常处理: try-except")
                super().visit_Try(node)
            
            def visit_Return(self, node):
                self.nodes.append(f"返回语句")
        ...
        # 生成CFG表示
        cfg_repr = "控制流图（CFG）表示:\n\n"
        cfg_repr += "节点列表:\n"
        for i, node in enumerate(visitor.nodes, 1):
            cfg_repr += f"  {i}. {node}\n"
        
        return cfg_repr
    except Exception as e:
        return f"CFG生成失败: {str(e)}"
...
def validate_code_complexity(src, expected_complexity, api_key=None, base_url=None):
    ...
    3. 不要输出任何解释或额外信息，只输出复杂度术语本身，如constant、linear、logn、nlogn、quadratic、cubic、exponential"""
    
    # 生成AST和CFG表示
    ast_repr = generate_ast(src)
    cfg_repr = generate_cfg_representation(src)
...
#     print(f"详细记录: {json.dumps(result, ensure_ascii=False, indent=2)}")
    ...
    # 示例2：从JSONL文件批量验证（已注释）
    print("\n=== 示例2: 从JSONL文件批量验证 ===")
    jsonl_path = "d:/MyResearch/codeComplex/data/python_data_cleaned.jsonl"
