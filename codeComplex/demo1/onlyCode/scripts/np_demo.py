...
import np_config  # 假设你有一个 np_config.py 文件
...
def process_code_file(code_path, force_reanalyze=False):
    ...
    result_dir = f"{np_config.np_results_base_dir}/results_{base_name}"
    ...
    if not force_reanalyze and os.path.exists(temp_results_path):
        ...
        start_n = int(test_results[-1][0]) + np_config.step if test_results else np_config.start_n
    else:
        start_n = np_config.start_n

    if start_n <= np_config.max_n:
        print(f"开始分析 {base_name} (n={start_n} -> {np_config.max_n})...")
        try:
            for n in range(start_n, np_config.max_n + 1, np_config.step):
...
def main():
    folder_path = np_config.np_folder_path
