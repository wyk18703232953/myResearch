...
def process_code_file(code_path, complexity_type="unknown", force_reanalyze=False):
    ...
    results_base_dir = results_base_map.get(complexity_type, config.quadratic_results_base_dir)
    result_dir = f"{results_base_dir}/results_{base_name}"
...
def main():
    ...
    # 收集所有路径下的 Python 文件
    for folder_name, folder_path in folder_configs:
        if os.path.exists(folder_path):
            python_files = sorted([os.path.join(folder_path, f) for f in os.listdir(folder_path) if f.endswith('.py')])
            for file_path in python_files:
                all_files.append((file_path, folder_name, force_reanalyze))
            print(f"在 {folder_name} 路径下找到 {len(python_files)} 个文件")
        else:
    ...
    for i, (file_path, complexity_type, force_reanalyze) in enumerate(all_files, 1):
        try:
            print(f"\n--- 处理文件 {i}/{len(all_files)}: {os.path.basename(file_path)} ---")
            process_code_file(file_path, complexity_type=complexity_type, force_reanalyze=force_reanalyze)
            success = True
            message = "处理成功"
        except Exception as e:
            error_msg = str(e)
            print(f"处理文件 {os.path.basename(file_path)} 时出错: {error_msg}")
            success = False
            message = error_msg
        ...
        print(f"\n进度: {completed_count}/{len(all_files)} - {os.path.basename(file_path)} {'✓' if success else '✗'}")
...
if __name__ == "__main__":
    main()
    print(f"程序完成时间是: {time.strftime('%H:%M:%S')}")
