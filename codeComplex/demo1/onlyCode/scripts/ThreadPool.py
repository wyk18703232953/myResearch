...
def process_code_file(code_path, force_reanalyze=False):
    ...
    result_dir = f"{config.quadratic_results_base_dir}/results_{base_name}"
...
def process_file_wrapper(args):
    """并行处理的包装函数"""
    path, force_reanalyze = args
    try:
        print(f"\n--- 处理文件: {os.path.basename(path)} ---")
        process_code_file(path, force_reanalyze=force_reanalyze)
        return (path, True, "处理成功")
    except Exception as e:
        error_msg = str(e)
        print(f"处理文件 {os.path.basename(path)} 时出错: {error_msg}")
...
def main():
    ...
    for folder_name, folder_path in folder_configs:
        if os.path.exists(folder_path):
            ...
            for file_path in python_files:
                all_files.append((file_path, force_reanalyze))
    ...
    print(f"使用 {max_workers} 个并发线程处理文件")
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        # 提交所有任务
        future_to_file = {executor.submit(process_file_wrapper, file_args): file_args[0] for file_args in all_files}
