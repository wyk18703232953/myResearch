...
def execute_with_timeout(code_str, globals_dict, timeout=120):
    """
    带超时的代码执行函数
    """
...
def process_code_file(code_path, folder_type, force_reanalyze=False):
    ...
    # NP 类型需要特殊处理（位运算会导致内存问题）
    np_max_n = 20 if folder_type == 'np' else config.max_n
    np_step = 2 if folder_type == 'np' else config.step
    np_start_n = 5 if folder_type == 'np' else config.start_n
    
    effective_max_n = min(np_max_n, config.max_n)
    effective_step = np_step
    effective_start_n = np_start_n
    
    if not force_reanalyze and os.path.exists(temp_results_path):
        with open(temp_results_path, 'r', encoding='utf-8') as f:
            test_results = json.load(f)
        start_n = int(test_results[-1][0]) + effective_step if test_results else effective_start_n
    else:
        start_n = effective_start_n
    
    if start_n <= effective_max_n:
        print(f"开始分析 {base_name} (n={start_n} -> {effective_max_n}, step={effective_step})...")
        try:
            for n in range(start_n, effective_max_n + 1, effective_step):
                # 构建测试代码
...
"""
                ...
                try:
                    ...
                    print(f"  原代码执行失败: {e}")
                    # 尝试移除代码中的 if __name__ == "__main__" 部分
                    ...
                    for line in lines:
                        ...
                        if in_main_block and line.strip() == '':
                            continue
                        if in_main_block and not line.startswith(' '):
                            in_main_block = False
                    ...
                    try:
                        ...
                        print(f"  移除 main block 后成功执行")
                    except Exception as e2:
                        print(f"  过滤后执行仍然失败: {e2}")
                        raise Exception(f"原代码执行失败: {e}")
                
                # 然后执行测试代码
                success, error_msg = execute_with_timeout(test_code, exec_globals, timeout=200)
...
def main():
    ...
    folder_types = getattr(config, 'all_folders', [ 'np'])
    # folder_types = getattr(config, 'all_folders', ['linear', 'constant'])
