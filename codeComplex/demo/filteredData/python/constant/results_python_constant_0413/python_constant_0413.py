...
def generate_points_from_n(n):
    base1 = n
    base2 = n * 2 + 1
    a_raw = [base1 + i for i in range(8)]
    b_raw = [base2 + (i * ((n % 5) + 1)) for i in range(8)]
    a_pts = [Point(a_raw[i], a_raw[i + 1]) for i in range(0, 8, 2)]
    b_pts = [Point(b_raw[i], b_raw[i + 1]) for i in range(0, 8, 2)]
    return a_pts, b_pts
...
def build_bb(b):
    ...
    for p in b:
        if p.x < bc.x:
            ...
            raise ValueError("Degenerate configuration for bb")
    return bb
...
def main(n):
    if n <= 0:
        n = 1
    a_pts, b_pts = generate_points_from_n(n)
    bb = build_bb(b_pts)
    res = solve_sqr_sqr45(a_pts, bb)
    print(["NO", "YES"][res])
