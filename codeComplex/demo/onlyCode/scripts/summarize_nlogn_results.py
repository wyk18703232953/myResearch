...
# 定义nlogn结果目录路径
results_dir = "d:/MyResearch/codeComplex/demo/onlyCode/results/nlogn"
...
def main():
    ...
    # 遍历所有results_python_nlogn_*目录
    result_dirs = glob.glob(os.path.join(results_dir, "results_python_nlogn_*"))
    ...
    for dir_path in result_dirs:
        ...
        if "python_nlogn_" in dir_name:
            problem_id = dir_name.split("python_nlogn_")[1]
        else:
            problem_id = dir_name
        ...
        if best_model:
            ...
            # 统计n_log_n模型数量
            if best_model == 'n_log_n':
                results_summary['nlogn_count'] += 1
            else:
                results_summary['non_nlogn_count'] += 1
                print(f"问题 {problem_id} 未识别为n_log_n，最佳模型是: {best_model} (R²={best_r_squared:.6f})")
        else:
            # 记录失败的文件
    ...
    if results_summary['total_files'] > 0:
        accuracy = results_summary['nlogn_count'] / results_summary['total_files']
    else:
        accuracy = 0
...
总体统计:
- 总分析文件数: {total_files}
- 识别为n_log_n的文件数: {nlogn_count}
- 未识别为n_log_n的文件数: {non_nlogn_count}
...
"""
    ...
    for result in results_summary['results']:
        if result['best_model'] != 'n_log_n':
            non_nlogn_list += f"- 问题 {result['problem_id']}: 最佳模型={result['best_model']}, R²={result['r_squared']:.6f}\n"
    
    if not non_nlogn_list:
        non_nlogn_list = "无"
    ...
    report_content = summary_report.format(
        ...
        nlogn_count=results_summary['nlogn_count'],
        non_nlogn_count=results_summary['non_nlogn_count'],
        failed_count=len(results_summary['failed_files']),
        accuracy=accuracy,
        non_nlogn_list=non_nlogn_list,
    ...
    print(f"- 识别为n_log_n的文件数: {results_summary['nlogn_count']}")
