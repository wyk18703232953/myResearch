...
# 读取原始代码
with open(r'd:\MyResearch\codeComplex\data\onlyCode\python\cubic\python_cubic_0001.py', 'r', encoding='utf-8') as f:
...
# 检查是否已经有部分结果
temp_results_path = r'D:/MyResearch/codeComplex/demo/onlyCode/python/cubic/results/temp_results.npz'
...
try:
    ...
    for n in range(start_n, max_n + 1, step):
        ...
        # 修改代码中的输入部分
        modified_code = re.sub(r'n, k = map\(int, input\(\).split\(\)\)', 
                             f'n, k = {n}, {k}', 
                             original_code)
    ...
    # 保存最终结果
    final_results_path = r'D:/MyResearch/codeComplex/demo/onlyCode/python/cubic/results/time_results.npz'
...
if len(test_results) >= 3:
    ...
    if fit_results:
        ...
        plt.legend()
        plt.grid(True)
        plot_path = r'D:/MyResearch/codeComplex/demo/onlyCode/python/cubic/results/time_complexity_plot.png'
        ...
        plt.legend()
        plt.grid(True)
        comparison_path = r'D:/MyResearch/codeComplex/demo/onlyCode/python/cubic/results/all_models_comparison.png'
        plt.savefig(comparison_path)
        plt.close()
        print(f'所有模型对比图已保存到 {comparison_path}')
        
        # 生成报告文件
        report_path = r'D:/MyResearch/codeComplex/demo/onlyCode/python/cubic/results/time_complexity_report.txt'
        with open(report_path, 'w', encoding='utf-8') as f:
            f.write('Time Complexity Analysis Report\n')
            f.write('=' * 50 + '\n')
            f.write(f'分析的代码文件: python_cubic_0001.py\n')
