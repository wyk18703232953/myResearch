...
def process_code_file(code_path, expected_models, base_dir):
    ...
    # 数据规模(x_data)越大，权重越高，强制模型在渐进复杂度的尾部更精确
    # -------------------------------------------------------------
    weights = x_data / np.max(x_data)
    # sigma 越小，拟合权重越大，防止除以 0 添加微小偏移量
    fit_sigma = 1.0 / np.maximum(weights, 1e-10) 
    
    for name, func in MODELS.items():
        try:
            # 引入加权参数 sigma=fit_sigma
            popt, pcov = curve_fit(func, x_data, y_data, sigma=fit_sigma, absolute_sigma=False, maxfev=10000)
            y_pred = func(x_data, *popt)
            
            if name == "Constant":
                # 对于常数分布，计算加权均值和加权方差
                pred_mean = np.average(y_pred, weights=weights)
                weighted_mean = np.average(y_data, weights=weights)
                ss_res = np.sum(weights * (y_data - pred_mean) ** 2)
                ss_tot = np.sum(weights * (y_data - weighted_mean) ** 2)
