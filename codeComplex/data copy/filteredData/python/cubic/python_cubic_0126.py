def main(n):
    ...
    if n <= 0:
        return
    # Interpret n as number of test cases; for complexity experiments we also scale string lengths with n.
    T = n
    for k in range(1, T + 1):
        len_s = max(1, k)
        len_t = max(1, (k % (k + 1)) + k // 2)
        s = ''.join(chr(ord('a') + (i * 7 + 3) % 26) for i in range(len_s))
        t = ''.join(chr(ord('a') + (i * 5 + 11) % 26) for i in range(len_t))
        solve_one(s, t)
